...
public class MatchService {
    ...
    public List<MatchesAndClubLogo> getlatestMatch() {
        List<Object[]> results = matchRepository.latestmatch();
        List<MatchesAndClubLogo> latestMatches = new ArrayList<>();
        for (Object[] result : results) {
            Long matchId = (Long) result[0];
            String matchDateTime = (String) result[1];
            int roundNo = (int) result[2];
            ...
            int awayTeamScore = (int) result[8];
            String stadiumName = (String) result[9];
            MatchesAndClubLogo match2 = new MatchesAndClubLogo(matchId, matchDateTime, roundNo, homeTeamName, homeTeamLogo,
                    homeTeamScore, awayTeamName, awayTeamLogo, awayTeamScore, stadiumName);
            latestMatches.add(match2);
